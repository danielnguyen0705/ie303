    ...
    <div className="min-h-screen bg-[#f7f9fc] flex">
      ...
      <aside className="w-[270px] h-screen fixed left-0 top-0 bg-[#1a1a2e] shadow-xl flex flex-col z-50 overflow-y-auto">
        <div className="px-6 py-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white tracking-tight uppercase">UIFIVE</h1>
              <p className="text-[10px] text-slate-400 font-medium tracking-[0.2em] uppercase">Admin Console</p>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 mt-4">
          {navigation.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={`mr-4 ${active ? 'opacity-100' : ''}`} size={20} />
                <span className="font-inter text-[13px] font-medium tracking-wide uppercase">{item.name}</span>
              </Link>
            );
          })}
        </nav>
        ...
        <div className="p-6">
          <div className="bg-white/5 rounded-lg p-4">
            <p className="text-[11px] text-slate-500 uppercase tracking-widest mb-2">System Load</p>
            <div className="w-full bg-white/10 h-1.5 rounded-full overflow-hidden">
              <div className="bg-[#b02d21] h-full w-[42%]"></div>
            </div>
            <p className="text-[10px] text-slate-400 mt-2">v2.4.0 Stable Build</p>
          </div>
        </div>
      </aside>
      ...
      <div className="ml-[270px] flex-1 flex flex-col">
        ...
        <header className="h-[60px] fixed top-0 right-0 w-[calc(100%-270px)] flex justify-between items-center px-6 bg-white shadow-sm z-40">
          ...
          <div className="flex items-center gap-6">
            <nav className="flex items-center gap-6">
              <Link className="text-red-700 font-bold font-inter text-sm cursor-pointer" to="/admin">
                Dashboard
              </Link>
            </nav>
            <div className="flex items-center gap-3">
              <HelpCircle className="text-slate-500 hover:text-red-600 cursor-pointer" size={20} />
              <Settings className="text-slate-500 hover:text-red-600 cursor-pointer" size={20} />
              ...
              <div className="flex items-center gap-3 cursor-pointer">
                ...
                <span className="text-sm font-semibold text-slate-900">Admin.UI</span>
              </div>
            </div>
          </div>
        </header>
        ...
        <footer className="py-3 flex justify-between items-center px-6 bg-slate-50 border-t border-slate-100">
          <p className="text-[11px] font-medium text-slate-400">UIFIVE Admin Panel v2.4.0</p>
          <div className="flex gap-4">
            <a className="text-[11px] font-medium text-slate-400 hover:text-red-500" href="#">Support</a>
            <a className="text-[11px] font-medium text-slate-400 hover:text-red-500" href="#">Privacy Policy</a>
          </div>
        </footer>
      </div>
    </div>
