...
import { faSeedling, faLock, faFire } from "@fortawesome/free-solid-svg-icons";
...
export function LessonSelection() {
      ...
      try {
        ...
        if (res.success) {
          const sorted = [...(res.data ?? [])].sort(
            (a, b) =>
              a.lessonNumber - b.lessonNumber || a.lessonId - b.lessonId,
          );
          setLessons(sorted);
        } else {
  ...
  const layout = useMemo(() => {
    ...
    const positionedLessons: PositionedLesson[] = lessons.map(
      (lesson, index) => ({
        ...lesson,
      ...
      <section className="mb-10">
        ...
        <div className="mt-4">
          ...
          <p className="text-gray-600 mt-2 text-lg">
            Lesson đang học sẽ màu cam, hoàn thành sẽ màu xanh, chưa mở khóa sẽ
            màu xám.
          </p>
        </div>
      </section>
