  ...
  return (
    <PracticePackageRunner<GroupReviewResponse>
      ...
      pageDescription={copy(
        ...
        "Các gói này kết hợp câu hỏi từ nhiều unit. Tài khoản VIP có thể mở lại và luyện như một bài ôn tập tập trung.",
      )}
      ...
      emptyListText={copy("No group reviews available yet.", "Chưa có gói ôn tập nhóm nào.")}
      emptyQuestionsText={copy(
        "This group review package has no questions yet.",
        "Gói ôn tập nhóm này chưa có câu hỏi.",
      )}
      ...
      getItemMeta={(item) =>
        `${gradeTitles[item.gradeId] ?? `${copy("Grade", "Lớp")} ${item.gradeId}`} - ${copy("Units", "Unit")} ${item.startUnit} ${copy("to", "đến")} ${item.endUnit} - ${item.questionIds.length} ${copy("question(s)", "câu hỏi")}`
      }
    />
  );
}
